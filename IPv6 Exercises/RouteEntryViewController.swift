//
//  RouteEntryViewController.swift
//  IPv6 Exercises
//
...
class RouteEntryViewController: UIViewController {
    ...
    let netPrefixTuple = NetPrefixTuple()
    var games = 0
    var score = 0
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        setFontSize()
        games = prefs.integer(forKey: "preGames")
        score = prefs.integer(forKey: "preScore")
        gamesPrint()
        preGenSet()
    }
    ...
    func preGenSet() {
        ...
        netPrefixTuple.generate()
        ipAddressValueLabel.text = netPrefixTuple.ipv6Address.string()
        for i in 0 ... 3 {
            let title = netPrefixTuple.netPrefix[i].string() + "/" + String(netPrefixTuple.netPrefix[i].prefixLength)
            buttonArray[i].setTitle(title, for: UIControlState.normal)
        }
        
        // debug
        print("Answer: \(netPrefixTuple.right)")
        // debug end
    ...
    func setFontSize() {
        ...
        //ipAddressValueLabel.font = UIFont.systemFont(ofSize: mySmallFontSize)
    ...
    override func viewDidDisappear(_ animated: Bool) {
        let prefs = UserDefaults.standard
        prefs.setValue(games, forKey: "preGames")
        prefs.setValue(score, forKey: "preScore")
    }
